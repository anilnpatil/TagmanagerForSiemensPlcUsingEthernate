...
RETRY_DELAY = 0.005  # 5ms delay
PLC_CONNECTION_POOL_SIZE = 2  # Only 2 connections
MAX_WORKERS = 1  # Sequential processing works better for S7-1200
DB_READ_CHUNK_SIZE = 128  # Bytes to read at once
WRITE_TIMEOUT = 0.3  # 300ms timeout
READ_TIMEOUT = 0.3  # 300ms timeout
MIN_WRITE_DELAY = 0.002  # 2ms between writes
TAG_CONFIG = {
...
logger = logging.getLogger("S7-1200-UltraFast")

# --------------------------
# Connection Pool with Multi-threading
# --------------------------
class PLCConnectionPool:
    ...
    def get_connection(self, ip=PLC_IP, timeout=1.0):
        start_time = time.time()
        while time.time() - start_time < timeout:
            with self.lock:
                if not self.pool.empty():
                    conn = self.pool.get()
                    if conn and self._check_connection(conn):
                        return conn
            time.sleep(0.005)
...
# --------------------------
def _execute_single_write(plc, db, offset, dtype, value, max_length):
    """Execute a single write operation with minimal overhead"""
    try:
        if dtype == 'DINT':
...
# Modified ultra_fast_write_operations function
def ultra_fast_write_operations(write_requests, plc_ip):
    """Single-threaded optimized write with minimal delays"""
    results = {}
    plc = plc_pool.get_connection(plc_ip)
    if not plc:
        return {item['tag']: {"status": "Failed", "error": "Connection failed"} 
    ...
    try:
        for item in write_requests:
            ...
            # Immediate write attempt with single retry
            for attempt in range(MAX_RETRIES + 1):
                try:
                    success = _execute_single_write(
                        plc,
                        config['db'],
                        config['offset'],
                        config['dtype'],
...
# --------------------------
def _execute_single_read(plc, db, offset, dtype, max_length):
    """Execute a single read operation with minimal overhead"""
    try:
        if dtype == 'REAL':
            ...
            return data[2:2+data[1]].decode('ascii').strip('\x00')
        return None
    except Exception as e:
        logger.error(f"Read error: {str(e)}")
        return None

def ultra_fast_batch_read(tag_configs, plc_ip):
    """Area-based reading with smart offset calculation"""
    results = {}
    plc = plc_pool.get_connection(plc_ip)
    if not plc:
        return {tag: None for tag in tag_configs.keys()}
    
    try:
        # Group by DB number and data type
        db_groups = defaultdict(lambda: defaultdict(list))
        for tag, config in tag_configs.items():
            db_groups[config['db']][config['dtype']].append((tag, config))
        
        # Process each DB and data type group
        for db, type_groups in db_groups.items():
            for dtype, tags in type_groups.items():
                if dtype in ['REAL', 'DINT']:
                    # Process 4-byte values together
                    offsets = [cfg['offset'] for _, cfg in tags]
                    min_offset = min(offsets)
                    max_offset = max(offsets) + 4
                    length = max_offset - min_offset
                    
                    try:
                        data = plc.db_read(db, min_offset, length)
                        for tag, config in tags:
                            offset = config['offset'] - min_offset
                            if dtype == 'REAL':
                                results[tag] = round(struct.unpack('>f', bytes(data[offset:offset+4]))[0], 2)
                            else:  # DINT
                                results[tag] = int.from_bytes(data[offset:offset+4], byteorder='big', signed=True)
                    except Exception as e:
                    ...
                    # Process strings individually (they vary in length)
                    for tag, config in tags:
                        try:
                            str_length = config.get('max_length', MAX_STRING_LENGTH) + 2
                            data = plc.db_read(db, config['offset'], str_length)
                            results[tag] = data[2:2+data[1]].decode('ascii').strip('\x00')
                        except Exception as e:
                            results[tag] = None
                            logger.error(f"String read failed for {tag}: {str(e)}")
...
def insert_data_to_plc():
    """Ultra-fast write endpoint with guaranteed <3s response"""
    start_time = time.time()
    try:
        ...
        if not isinstance(data_list, list):
            return jsonify({"message": "Payload must be list"}), 400
        
        results = ultra_fast_write_operations(data_list, plc_ip)
        elapsed = time.time() - start_time
        
        success_count = sum(1 for r in results.values() if r.get('status') == 'Success')
        
        logger.info(f"Write completed in {elapsed:.3f}s - Success: {success_count}/{len(data_list)}")
...
def get_tag_values():
    """Ultra-fast read endpoint with guaranteed <3s response"""
    start_time = time.time()
    try:
        plc_ip = request.args.get('ip') or PLC_IP
        ...
        if not isinstance(requested_tags, list):
            return jsonify({"message": "Tags must be list"}), 400
        
        # Filter valid tags
        tag_configs = {tag: TAG_CONFIG[tag] for tag in requested_tags if tag in TAG_CONFIG}
        
        results = ultra_fast_batch_read(tag_configs, plc_ip)
        elapsed = time.time() - start_time
        
        success_count = sum(1 for v in results.values() if v is not None)
        
        logger.info(f"Read completed in {elapsed:.3f}s - Success: {success_count}/{len(requested_tags)}")
...
def shutdown_plc_pool(exception=None):
    while not plc_pool.pool.empty():
        conn = plc_pool.pool.get()
        try:
            conn.disconnect()
        except:
            pass
