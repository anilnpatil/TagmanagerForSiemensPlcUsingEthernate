...
# PLC Connection
# -------------------------------
def get_plc_connection(ip=PLC_IP):
    global plc_connection
    with connection_lock:
        if plc_connection is None or not plc_connection.get_connected():
            try:
                if plc_connection:
...
def batch_write_plc(plc, tag_value_pairs):
    """
    Optimized batch write operation for PLC tags
    Groups writes by DB and combines adjacent writes where possible
    """
    from collections import defaultdict
    results = {}
    ...
    for db_num, write_list in grouped.items():
        # Sort writes by offset
        write_list.sort(key=lambda x: x[0])
        
        # Combine adjacent writes where possible
        combined_writes = []
        current_offset = None
        ...
        for offset, data, tag in write_list:
            # If this write is adjacent to the current block, extend it
...
def insert_data_to_plc():
    ...
    try:
        ...
        for item in data_list:
            if not isinstance(item, dict) or 'tag' not in item or 'value' not in item:
                continue
