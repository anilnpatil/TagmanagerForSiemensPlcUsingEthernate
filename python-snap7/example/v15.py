...
LAST_WRITE_TIME = 0
MIN_WRITE_INTERVAL = 0.1  # 100ms between writes
    ...
    with connection_lock:
        for attempt in range(max_retries):
            try:
                ...
                plc_connection.connect(ip, PLC_RACK, PLC_SLOT)
                plc_connection.set_connection_type(3)  # PG type connection (more stable)
                logger.info(f"Connected to PLC (Attempt {attempt+1})")
                return plc_connection
                
            except Exception as e:
                logger.warning(f"PLC connection attempt {attempt+1} failed: {str(e)}")
                if attempt < max_retries - 1:
                    time.sleep(0.5 * (attempt + 1))  # Exponential backoff
                continue
        
        logger.error("Failed to establish PLC connection after retries")
        return None

# -------------------------------
# Data Utilities
# -------------------------------
def validate_data_size(db_number, offset, data):
    if not (0 <= offset < MAX_DB_SIZE):
        raise ValueError(f"Offset {offset} out of bounds")
    if not (0 <= offset + len(data) <= MAX_DB_SIZE):
        raise ValueError(f"Data at offset {offset} exceeds DB bounds")
    ...
    try:
        if dtype == 'DINT':
            ...
            if not isinstance(value, str):
                value = str(value)
            max_len = config.get('max_length', MAX_STRING_LENGTH)
            str_bytes = value.encode('ascii', errors='ignore')[:max_len]
            str_len = len(str_bytes)
            data = bytearray(max_len + 2)
            data[0] = max_len      # max length
            data[1] = str_len      # actual string length
            data[2:2+str_len] = str_bytes  # actual string content

        else:
            raise ValueError(f"Unsupported data type: {dtype}")

        validate_data_size(db, offset, data)
...
def batch_read_plc(plc, tags):
    ...
    for db_num, tag_list in grouped.items():
        ...
        max_offset = max(cfg['offset'] + (cfg.get('max_length') + 2 if cfg['dtype'] == 'STRING' else 4)
                         for _, cfg in tag_list)
        ...
        for tag, cfg in tag_list:
            ...
            try:
                if cfg['dtype'] == 'REAL':
                    value = round(struct.unpack('>f', bytes(block[start:start + 4]))[0], 2)
                elif cfg['dtype'] == 'DINT':
                    value = int.from_bytes(block[start:start + 4], byteorder='big', signed=True)
                elif cfg['dtype'] == 'STRING':
                    length = block[start + 1]
                    value = block[start + 2:start + 2 + length].decode('ascii').strip('\x00')
                else:
                    value = None
                results[tag] = value
            except Exception as e:
...
def batch_write_plc(plc, tag_value_pairs, max_retries=2):
    from collections import defaultdict
    results = {}
    
    for attempt in range(max_retries):
        try:
            # Group tags by DB and prepare write operations
            grouped = defaultdict(list)
            ...
            # Process each DB's writes
            for db_num, write_list in grouped.items():
                write_list.sort(key=lambda x: x[0])  # Sort by offset
                ...
                for offset, data, tag in write_list:
                    if current_offset is not None and offset == current_offset + len(current_data):
                        ...
                        if current_offset is not None:
                            combined_writes.append((current_offset, current_data, current_tags))
                ...
                for offset, data, tags in combined_writes:
                    try:
                        ...
                        # On write error, mark these tags for retry
                        for tag in tags:
                            if tag in results:
                                del results[tag]
    ...
    for tag, _ in tag_value_pairs:
        if tag not in results:
            results[tag] = {"status": "Failed", "error": "All retries failed"}
    return results
...
def insert_data_to_plc():
    ...
    try:
        ...
        # Prepare tag-value pairs
        tag_value_pairs = [(item['tag'], item['value']) for item in data_list 
                          if isinstance(item, dict) and 'tag' in item and 'value' in item]

        # Execute batch write
        with connection_lock:
            results = batch_write_plc(plc, tag_value_pairs)

        logger.info(f"Write operation completed in {time.time()-start_time:.3f} seconds")
        return jsonify({
            "message": "Batch write completed",
            "execution_time": f"{time.time()-start_time:.3f} seconds",
            "data": results
        }), 200

    except Exception as e:
        logger.error(f"PLC write error: {str(e)}")
        return jsonify({
...
def get_tag_values():
    try:
        ...
        if not plc_ip or not requested_tags:
...
if __name__ == '__main__':
    try:
        get_plc_connection()
        app.run(host='0.0.0.0', port=8083, threaded=True)  # Use multithreaded Flask for responsiveness
    finally:
        if plc_connection:
            try:
